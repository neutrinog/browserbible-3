...
var uwGrabAvailableTexts = function() {
  ...
  uwObject.downloadBibles = function(bibles) {
    ...
    for (var i = 0; i < bibles.length; i++) {
       ...
       */
      writeFileDisplayError(directoryPath + '/info.json', JSON.stringify(bible.version_info));
      /**
       * Let's create the about.html file
       */
      writeFileDisplayError(directoryPath + '/about.html', bible.about);
      /**
  ...
  /**
   * Refactored this into a separate function because the error message was showing incorrect information since
   * the for loop had moved on and the value of fileName had changed before the message was displayed.
   * @param {string} fileName
   * @param {Object} data
     */
  function writeFileDisplayError(fileName, data) {

    // Even though this is the default value for options, there is a bug in "mode" value in the node module
    // noinspection OctalIntegerJS
    options = { encoding: 'utf8', mode: 0666, flag: 'w' };

    fileSystem.writeFile(fileName, data, options, function(error) {
      if (error) {
        display('Unable to create the info JSON file at: ' + fileName + ' because: ' + error);
      }
    });
  }
